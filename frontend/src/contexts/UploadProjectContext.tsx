...
import { ProjectStatus, Imedia, TempMedia, ProjectUploadType, Thumbnail } from "@/types/project";
...
const UploadProjectContext = createContext<UploadProjectContextType | undefined>(undefined);
...
export const UploadProjectProvider: React.FC<UploadProjectProviderProps> = ({ children, projectID }) => {
    const {
        projectMetadata,
        tags,
        tools,
    ...
    const { initialMedia, newMedia, initialThumbnail, newThumbnail } = useMediaUpload();
    const { user } = useUser();
...
export const useUploadProject = (): UploadProjectContextType => {
    ...
    if (!context) {
        throw new Error("useUploadProject must be used within an UploadProjectProvider");
    }
    return context;
};