        ...
        >
            <Link href={`/project/${project._id}`} className="w-full">
                <div className="relative h-64 overflow-hidden rounded-none ">
                    ...
                    {hovered || styles?.includes("pointer-events-none") && (
                        <CardHeader
                            className="absolute w-full h-36 bottom-0 bg-gradient-to-t from-black via-black/60 to-transparent group-hover:opacity-100 transition-opacity duration-300">
            ...
            <CardContent className="px-4 py-1 mt-1">
                <div className="flex justify-between">
                    {!isOwner && renderUser && (
                        <MiniUserInfo id={project?.creator?._id} avatar={project?.creator?.profile?.avatar}
                                      styles="w-3/4" fullName={project?.creator?.fullName}/>
                    )}
                    <div className="flex space-x-4 justify-between flex-grow">
                        <div className="flex items-center space-x-1">
                            <Eye size={16}/>
                            <span className={`text-sm`}>{project.stats.views.toLocaleString()}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                            <Heart size={16} className="text-primary"/>
                            <span className={`text-sm`}>{project.stats.likes.toLocaleString()}</span>
                        </div>
                    </div>
