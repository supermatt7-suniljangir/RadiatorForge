...
// import { ICopyright, IStats, ProjectStatus, License } from "@/types/project";
...
//     updateProjectMetadata: (data: Partial<Omit<ProjectMetadata, 'thumbnail'>>) => void;
//     uiState: UIState;
...
// export const ProjectProvider: React.FC<ProjectProviderProps> = ({ children, initialData = {} }) => {
//     // Initialize project metadata without thumbnail.
//     const [projectMetadata, setProjectMetadata] = useState<Omit<ProjectMetadata, 'thumbnail'>>({
...
//     const updateProjectMetadata = (data: Partial<Omit<ProjectMetadata, 'thumbnail'>>) => {
...
//                 copyRight,
//                 updateCopyRight: (data) => setCopyRight((prev) => ({ ...prev, ...data })),
//                 editorStage,
//                 updateEditorStage: setEditorStage,
//                 collaborators,
//                 updateCollaborators: setCollaborators,
//             }}
//         >
//             {children}
//         </ProjectContext.Provider>
...
export const ProjectProvider: React.FC<ProjectProviderProps> = ({ children, initialData = {} }) => {
    ...
    const [editorStage, setEditorStage] = useState<0 | 1 | 2>(initialData?.editorStage || 0);
    const [uiState, setUIState] = useState<UIState>(
        initialData?.uiState || {
            isImageLoading: false,
            isDescOpen: !!(initialData?.description || initialData?.shortDescription),
    ...
    const updateProjectMetadata = useCallback((data: Partial<Omit<ProjectMetadata, 'thumbnail'>>) => {
        setProjectMetadata((prev) => ({ ...prev, ...data }));
    }, []);
