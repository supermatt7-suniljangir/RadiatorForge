    ...
    const fetchUser = async () => {
        try {
            const {
                ...
                error,
                status,
            } = await getUserProfile({cacheSettings: "reload"}) as ApiResponse;
            if (!success) {
                console.log('error', error)
                if (status === 401 && error !== "AUTH_REQUIRED") {
                    // Only show toast for 401 errors if user was logged in
                    const toastMessage = toastMap[error] || "An unexpected error occurred.";
                    toast({variant: "destructive", description: toastMessage, title: "Error"});
                }
                await logout();
                router.push("/login");
                return;
